...
abstract contract QVBaseStrategy is BaseStrategy {
	...
	mapping(address => mapping(uint256 => mapping(Status => uint256)))
		public reviewsByStatus;

	// recipientId -> applicationId -> reviewer -> status
	mapping(address => mapping(uint256 => mapping(address => Status)))
		public reviewedByManager;
	...
	function reviewRecipients(
		...
		for (uint256 i; i < recipientLength; ) {
			...
			if (
				recipientStatus == Status.None || recipientStatus == Status.Appealed
			) {
				revert RECIPIENT_ERROR(recipientId);
			}

			if (
				reviewedByManager[recipientId][applicationId][msg.sender] > Status.None
			) {
				revert RECIPIENT_ERROR(recipientId);
			}

			// track the review cast for the recipient and update status counter
			reviewedByManager[recipientId][applicationId][
				msg.sender
			] = recipientStatus;
	...
	function _distribute(
		...
		bytes memory,
		address _sender
	) internal virtual override onlyPoolManager(_sender) onlyAfterAllocation {
		...
		for (uint256 i; i < payoutLength; ) {
			...
			// if (
			// 	paidOut[recipientId] ||
			// 	!_isAcceptedRecipient(recipientId) ||
			// 	amount == 0
			// ) {
			// 	revert RECIPIENT_ERROR(recipientId);
			// }
	...
	/// @notice Calcula los votos efectivos adicionales correspondientes a una cantidad adicional
	/// de créditos de voz para un recipiente, teniendo en cuenta los créditos previamente asignados.
	/// @param _allocatorAddress La dirección del allocator.
	/// @param _recipientId El ID del recipiente.
	/// @param _additionalVoiceCredits La cantidad de créditos de voz adicionales que se desean asignar.
	/// @return Los votos efectivos adicionales resultantes de asignar los créditos adicionales.
	function calculateAdditionalEffectiveVotes(
		address _allocatorAddress,
		address _recipientId,
		uint256 _additionalVoiceCredits
	) public view returns (uint256) {
		Allocator storage _allocator = allocators[_allocatorAddress];
		...
		uint256 creditsCastToRecipient = _allocator.voiceCreditsCastToRecipient[
			_recipientId
		];
		uint256 votesCastToRecipient = _allocator.votesCastToRecipient[
			_recipientId
		];

		// get the total credits and calculate the vote result
		uint256 totalCredits = _additionalVoiceCredits + creditsCastToRecipient;
	...
	function _qv_allocate(
		...
		_allocator.voiceCreditsCastToRecipient[
			_recipientId
	...
	function _getPayout(
		...
		if (!paidOut[_recipientId] && totalRecipientVotes != 0) {
			amount =
				(poolAmount * recipient.totalVotesReceived) /
				totalRecipientVotes;
		}
